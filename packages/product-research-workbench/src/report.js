...
export function renderMarkdown(snapshot) {
  ...
  for (const [index, source] of sources.entries()) {
    ...
    const locator = source.locator ? ` — ${source.locator}` : ' — pasted text (local only)'
...
export function renderHtml(snapshot) {
  ...
  const sourceItems = snapshot.sources.map((source, index) => {
    ...
    const reference = source.locator
      ? `<a href="${htmlText(source.locator)}" rel="noreferrer noopener">${htmlText(source.locator)}</a>`
...
<meta name="generator" content="DSH Toolbox Product Research Workbench Alpha">
...
<p class="eyebrow">DSH Toolbox · Product Research Workbench · Experimental Alpha</p>
